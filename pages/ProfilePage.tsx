...
const ProfilePage: React.FC = () => {
    ...
    const getValidityDateString = (membershipDateStr: string): string => {
        const joinDate = new Date(membershipDateStr);
        const joinYear = joinDate.getFullYear();
        const joinMonth = joinDate.getMonth(); // 0-indexed, so April is 3

        // If member joins from April 1st onwards, their membership expires March 31st of the next year.
        // If member joins Jan-Mar, their membership expires March 31st of the same year.
        const expiryYear = joinMonth >= 3 ? joinYear + 1 : joinYear;

        // Create the expiry date object: new Date(year, monthIndex, day)
        const expiryDate = new Date(expiryYear, 2, 31); // 2 is March
                    ...
                    <div ref={cardRef} className="bg-gradient-to-br from-primary to-purple-600 text-white p-6 rounded-xl shadow-2xl relative overflow-hidden">
                        ...
                        <div className="mt-4 flex justify-between items-end">
                            ...
                            <div>
                                ...
                                <p>{getValidityDateString(currentUser.membershipDate)}</p>
                            </div>
                        </div>
                    </div>
                     <button 
